...
class HrExpense(models.Model):
    ...
    rate_aux = fields.Float(compute='_compute_tasa')
...
class AccountGastoAnticipo(models.Model):
    ...
    journal_id = fields.Many2one('account.journal',domain="[('type', 'in', ('bank', 'cash'))]", string='Journal', required=True, readonly=True, states={'draft': [('readonly', False)]},company_dependent=True)
    ...
    amount = fields.Monetary(string='Monto Real Entregado', required=True, readonly=True, states={'draft': [('readonly', False)]}, tracking=True)
    currency_id = fields.Many2one('res.currency', string='Currency', required=True, readonly=True, states={'draft': [('readonly', False)]}, default=lambda self: self.env.company.currency_id,company_dependent=True)
    ...
    tasa_personalizada = fields.Boolean()
    rate = fields.Float(default=1)
    monto_moneda_company=fields.Float(compute='_compute_monto_company')
    ...
    def get_name(self):
        ...
        self.ensure_one()
        SEQUENCE_CODE = 'secuencia_anticipo_empleado'
        ...
        if not name:
            IrSequence.sudo().create({
                'prefix': 'Anticipo Viatico/',
                'name': 'Localización Venezolana Anticipo Empleado %s' % self.env.company.name,
                'code': SEQUENCE_CODE,
