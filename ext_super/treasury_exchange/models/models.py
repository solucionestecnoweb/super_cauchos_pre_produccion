...
class Exchange(models.Model):
	...
	final_currency_id = fields.Many2one ('res.currency' ,"Modeda Destino" )
	rate = fields.Float(string="Rate")

	journal_id = fields.Many2one('account.journal', "Diario")
	debit_id = fields.Many2one('account.journal', "Origen de Fondos")
	credit_id = fields.Many2one('account.journal',"Destino de Fondos")
	account_id = fields.Many2one('account.account',"Cuenta Transitoria")

	move_transient_id  = fields.Many2one('account.move',"Asiento Origen")
	...
	def confirmed(self):
		move_vals = {
				...
				'line_ids': [
					(0, 0, {
						'name': self.reference,
						'amount_currency': self.final_amount * -1 if self.final_currency_id != self.company_id.currency_id else 0 ,
						'currency_id': self.final_currency_id.id  if self.final_currency_id != self.company_id.currency_id else 0 ,
						'debit':  0,
						'credit': self.amount ,
						'partner_id': self.company_id.partner_id.id,
